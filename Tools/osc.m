...
.TP 13
.B \-o absolute
Name the final executable program \fIabsolute\fR.  The absolute
name cannot end in a sisal extension (.sis, .if1, .mono, .opt, 
.mem, .up, .part, .costs, .a, .c, .s, .o, .f, .i).
...
\-info=1,2,3,5 is the same as \-info=1-3,5 and is the same as \-info -info=2,3
\-info=5.
.br
\ \ \ \ 1 : General interest
.br
\ \ \ \ 2 : More information of general interest
.br
\ \ \ \ 3 : Detailed, source line information where appropriate
.br
\ \ \ \ 4 : Information about vectorization and parallelization
.br
The numbers 13-16 are reserved for developer's information which is
unlikely to be interesting.

.\" Machine generated entry.  Do not change
.TP 13
.B \-cvinfo
Print concurrentization and vectorization information gathered
during compilation to stderr.  This option provides a subset of the
information generated by the \-info option. Default: run silently.  

.\" Machine generated entry.  Do not change
.TP 13
.B \-cvinfo=<range list>
Print more levels of concurrentization and vectorization information.
\-cvinfo=0 produces no information, \-cvinfo=1 is the same as
\-cvinfo.
See \-info= for more information on range lists.
...
The default compiler behavior is to insure that all slices are at
least that thick.
If the estimate is bad, then true paralellism will be lost.
